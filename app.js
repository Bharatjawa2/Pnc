...
btnSendToVision.addEventListener('click', async () => {
  ...
  const VISION_SERVER = 'http://192.168.31.57:5002'; // adjust if needed
  const pingUrl = `${VISION_SERVER}/ping`;
  ...
  try {
    // quick sanity check: is server reachable?
    console.log('Pinging vision server at', pingUrl);
    let pingOk = false;
    try {
      const pingResp = await fetch(pingUrl, { method: 'GET', mode: 'cors' });
      if (pingResp.ok) { pingOk = true; console.log('Ping OK', pingResp.status); }
      else console.warn('Ping returned non-OK:', pingResp.status);
    } catch (pe) {
      console.warn('Ping failed (network/CORS?):', pe);
    }

    if (!pingOk) {
      const proceed = confirm(`Cannot reach ${VISION_SERVER} (ping failed). Continue and try sending anyway? (Check server/CORS)`);
      if (!proceed) return;
    }
